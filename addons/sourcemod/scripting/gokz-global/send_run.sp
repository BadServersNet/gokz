/*
	Sends a run to the global API together with its replay file.
	The replay file is owned by gokz-replays and is never deleted here.
*/
...
{
	if (!FileExists(storedReplayPath[client]))
	{
		LogError("Failed to upload replay to the global API. File \"%s\" no longer exists.", storedReplayPath[client]);
		lastRecordId[client] = -1;
